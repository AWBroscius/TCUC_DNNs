...
checkpath = os.path.join(location, 'model', sys.argv[3])

from keras.src.legacy.saving import legacy_h5_format

new_model = legacy_h5_format.load_model_from_hdf5(checkpath, custom_objects={'mse': 'mse'})

print("\n Using model from: ", sys.argv[3])
print(new_model)

# use trained model to make predictions
predictions = new_model.predict(Time_series_X_test)

print("Nonzero predictions: ", np.count_nonzero(predictions))
...
for threshold_inst in threshold:
    ...
    predictions_train = new_model.predict(Time_series_X_train)
